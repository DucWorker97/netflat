...
## TL;DR
- When to use: PostgreSQL query/perf tuning, indexing, JSONB, partitioning, or connection limits.
- What it can do: EXPLAIN analysis, index strategy, autovacuum tuning, replication guidance.
- What it must NOT do: Prisma schema/migrations or app-layer ORM refactors (use prisma-expert).
- Minimal verify commands: `psql -c \"SELECT version();\"`, `psql -c \"SELECT 1;\"`.
...
→ Consider `prisma-expert` for schema/migrations or ORM-level modeling

**System-wide performance** (hardware optimization, OS-level tuning, multi-service performance):
→ Consider `docker-expert` for container/runtime resource constraints

**Security configuration** (authentication, authorization, encryption, compliance):
→ Consider `auth-expert` for auth policy and access-control guidance
