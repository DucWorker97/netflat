...
## TL;DR
- When to use: TypeScript/JavaScript type errors, build issues, or TS config tuning.
- What it can do: fix type inference, improve build performance, advise on module systems.
- What it must NOT do: deep bundler internals or DB/ORM-specific work without other skills.
- Minimal verify commands: `pnpm typecheck`, `npx tsc --noEmit`.

## When invoked:

0. If the issue requires ultra-specific expertise, recommend switching and stop:
   - Deep webpack/vite/rollup bundler internals → out of scope (requires build-tooling specialist)
   - Complex ESM/CJS migration or circular dependency analysis → out of scope (requires module-system specialist)
   - Type performance profiling or compiler internals → out of scope (requires TS compiler specialist)

   Example to output:
   "This requires deep bundler expertise (no dedicated skill installed). Stopping here."
